...
public class NumPickView extends View {
        ...
        mValueAnimator.addListener(new Animator.AnimatorListener() {
            ...
            public void onAnimationEnd(Animator animation) {
                if (mStatus == Status.UP && pivot != 0) {
                    mCurrentPosition = clamp(mCurrentPosition + 1);
                ...
                mScale = 0;
                if (mListener != null) {
                    mListener.onSelected(mCurrentPosition);
                }
            }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        textStep = mUnitHeight / 5;
    }

    @Override
    protected void onDraw(Canvas canvas) {
        //画选中字体
        drawText(canvas, mData.get(mCurrentPosition), 0, 1);
        //画除中间外上下字体
        int num = mShowNum / 2;
        for (int i = 1; i <= num; i++) {
            drawText(canvas, mData.get(clamp(mCurrentPosition + i)), i, 1);
            drawText(canvas, mData.get(clamp(mCurrentPosition - i)), i, -1);
        }
    ...
    private void drawText(Canvas canvas, String text, int level, int direct) {
        ...
        if (level == 0) {
            ...
            canvas.drawText(text, middleWidht , mHeight / 2f + mRect.height() / 2f + pivot, mPaint);
        } else {
            //其他字体根据上下和滑动方向关系放大或缩小, 颜色渐变
            int color = (int) mArgvEvlauator.evaluate(1 - abs(mRect.height() / 2f + offset + pivot)/middleHeight
                    , Color.parseColor(DEF_START_COLOR)
                    , mTextColor);
            mPaint.setColor(color);
            mPaint.setTextSize(textSize - textStep * level + step);
            mPaint.getTextBounds(text, 0, text.length(), mRect);
            canvas.drawText(text, middleWidht , middleHeight + mRect.height() / 2f + offset + pivot, mPaint);
        }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_UP:
                ...
                if (abs(pivot) > mUnitHeight / 2f) {
                    //需要过渡
                    int rest = (int) abs(mUnitHeight / 2f - pivot);
                    if (mStatus == Status.UP) {
                        mValueAnimator.setFloatValues(pivot, -rest);
                    } else if (mStatus == Status.DOWN) {
                        //这里需要注意
                        mValueAnimator.setFloatValues(pivot, (int) pivot + rest + mUnitHeight / 2f);
                    }
                } else {
                    //过渡失败，返回原数值，所以终点都是 0
                    if (mStatus == Status.UP) {
    ...
    public int getPickNumber(){
        return Integer.parseInt(mData.get(mCurrentPosition));
    }
