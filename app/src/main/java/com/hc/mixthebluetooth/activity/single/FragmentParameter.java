...
public class FragmentParameter {
    ...
    public void setCodeFormat(String code, Context context){
        if (mStorage == null) mStorage = new Storage(context);
        mCodeFormat = code;
        mStorage.saveCodedFormat(code);
    }

    public String getCodeFormat(Context context){
        if (mStorage == null && context != null) mStorage = new Storage(context);

        if (mCodeFormat == null && mStorage != null) mCodeFormat = mStorage.getCodedFormat();

        if (mCodeFormat == null) return "GBK";
