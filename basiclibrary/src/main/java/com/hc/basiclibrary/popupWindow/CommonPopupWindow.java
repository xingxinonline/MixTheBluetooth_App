package com.hc.basiclibrary.popupWindow;
...
public class CommonPopupWindow extends PopupWindow{
    ...
    public CommonPopupWindow(int layout, View view, int width,int height){
        ...
        super(LayoutInflater.from(view.getContext()).inflate(layout, null),
                decideTransition(view.getContext(),width),
                decideTransition(view.getContext(),height), true);
    ...
    public <T extends View> T findViewById(int view){//绑定View
        if (getContentView() != null) return getContentView().findViewById(view);
        return null;
    }

    public void setListeners(View.OnClickListener listener,int...ids){//设置View的按钮监听
        if (getContentView() == null) {
            Log.e("AppRun"+getClass().getSimpleName(),"错误：mContentView 为 null,即所依附View不存在...");
            return;
        }
        for (int id : ids) {
    ...
    public class Builder{
         ...
         */
        public Builder setExcursion(Context context,float horizontalExcursion,float verticalExcursion){
            this.horizontalExcursion = dip2px(context,horizontalExcursion);
            this.verticalExcursion = dip2px(context,verticalExcursion);
            return this;
        }
        ...
        public CommonPopupWindow create(){
            ...
            if (Build.VERSION.SDK_INT < Build.VERSION_CODES.O){
                //不知为何，小于Android 8.0的 需要再次设置WRAP_CONTENT，否则有错误
    ...
    /**
     * 判断是否需要转换 MATCH_PARENT WRAP_CONTENT 不需要转换
     * @param context context
     * @param dpValue MATCH_PARENT WRAP_CONTENT 或是一个正数
     */
    private static int decideTransition(Context context,float dpValue){
        if (dpValue == ViewGroup.LayoutParams.MATCH_PARENT) return (int) dpValue;
        if (dpValue == ViewGroup.LayoutParams.WRAP_CONTENT) return (int) dpValue;
