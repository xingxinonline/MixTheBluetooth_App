...
public class ModuleParameters {
    ...
    private static boolean isCheckNewline = true;//是否检查接收的数据包含换行，并根据其换行加载缓存
    ...
    private static boolean isSendFile = false;//是否传输文件

    private static int sendFileDelayedTime = 0;//发送文件时的延时
    ...
    public static void setParameters(int state, int bleReadBuff, int classicReadBuff,int bleTime, Context context) {
        if ( ModuleParameters.state != state || ModuleParameters.bleReadBuff != bleReadBuff || ModuleParameters.classicReadBuff != classicReadBuff || ModuleParameters.time != bleTime) {
            ...
            if (bleReadBuff<20) bleReadBuff = 20;
            ModuleParameters.bleReadBuff = bleReadBuff;
            if (classicReadBuff<20) classicReadBuff = 20;
            ModuleParameters.classicReadBuff = classicReadBuff;
            if (bleTime<20) bleTime = 20;
    ...
    public static void setSendFile(boolean isSendFile) {
        ModuleParameters.isSendFile = isSendFile;
    }
    ...
    public static boolean isCheckNewline(){return isCheckNewline;}
    ...
    public static int getSendFileDelayedTime() {
        return sendFileDelayedTime;
    }

    public static void setSendFileDelayedTime(int delayedTime){
        sendFileDelayedTime = delayedTime;
    }
